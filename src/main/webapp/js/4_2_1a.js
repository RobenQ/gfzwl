				...
				storeId.rows[i].style.display='';
					...
					storeId.rows[i].style.display='';//显示行操作，
						...
						storeId.rows[i].style.display='';//显示行操作，
							...
							storeId.rows[i].style.display='';//显示行操作，
							}else if(key==6&&searchText=="第六章"){//用match函数进行筛选，如果input的值，即变量 key的值为空，返回的是ture，
								storeId.rows[i].style.display='';//显示行操作，
								}else if(key==7&&searchText=="第七章"){//用match函数进行筛选，如果input的值，即变量 key的值为空，返回的是ture，
									storeId.rows[i].style.display='';//显示行操作，
								}else if(key==8&&searchText=="第八章"){//用match函数进行筛选，如果input的值，即变量 key的值为空，返回的是ture，
									storeId.rows[i].style.display='';//显示行操作，
								}else{
...
function download(btn){
	window.location.href="/gfzwl/user/downfile";
}
...
function download2(btn){
	...
	http.open("GET", "/gfzwl/admin/deletefile4_2_1a?path="+path+"&name="+name+"&type="+type, true);
//	http.setRequestHeader("Content-type", "application/x-www-form-urlencoded;");
	http.send();
	http.onreadystatechange = function() {
		if(http.readyState == 4 && http.status == 200) {
			window.location.href="/gfzwl/admin/deletefile4_2_1a?path="+path+"&name="+name+"&type="+type;
		}
	}
}