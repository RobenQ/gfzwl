...
function download2(btn){
	...
	http.open("GET", "/gfzwl/user/downfile?path="+path+"&name="+name, true);
	http.setRequestHeader("Content-type", "application/x-www-form-urlencoded;");
	http.send();
	http.onreadystatechange = function() {
		if(http.readyState == 4 && http.status == 200) {
			window.location.href="/gfzwl/user/downfile?path="+path+"&name="+name;	
		}
	}
}
...
function showVideo(btn){
	...
	http.open("GET", "/gfzwl/user/showvideo?path="+path+"&name="+name, true);
	http.setRequestHeader("Content-type", "application/x-www-form-urlencoded;");
	http.send();
	http.onreadystatechange = function() {
		if(http.readyState == 4 && http.status == 200) {
			window.location.href="/gfzwl/user/showvideo?path="+path+"&name="+name;	
		}
	}
}